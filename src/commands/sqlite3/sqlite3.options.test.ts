...
describe("sqlite3 options", () => {
  describe("-version", () => {
    it("should show SQLite version", async () => {
      const env = new Bash();
      const result = toText(await env.exec("sqlite3 -version"));
  ...
  describe("-- end of options", () => {
    it("should treat arguments after -- as positional", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec('sqlite3 :memory: -- "SELECT 1 as value"'),
      );
  ...
  describe("-echo", () => {
    it("should print SQL before execution", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec('sqlite3 -echo :memory: "SELECT 1; SELECT 2"'),
      );
  ...
  describe("-readonly", () => {
    it("should not persist changes with -readonly", async () => {
      ...
      const result = toText(await env.exec('sqlite3 /ro.db "SELECT * FROM t"'));
      expect(result.stdout).toBe("1\n");
      expect(result.exitCode).toBe(0);
    });
  });
  ...
  describe("-bail", () => {
    it("should stop on first error with -bail", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec('sqlite3 -bail :memory: "SELECT * FROM bad; SELECT 1"'),
      );
