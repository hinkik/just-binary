...
describe("paste command", () => {
  ...
  describe("basic functionality", () => {
    it("should paste two files side by side with tab delimiter", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste file1.txt file2.txt"));
    ...
    it("should handle files with uneven line counts", async () => {
      const env = createEnv();
      const result = await toText(
        await env.exec("paste uneven1.txt uneven2.txt"),
      );
      expect(result.stdout).toBe("a\t1\nb\t2\n\t3\n\t4\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
    ...
    it("should handle single file", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste file1.txt"));
  ...
  describe("-d (delimiter)", () => {
    it("should use custom delimiter", async () => {
      const env = createEnv();
      const result = await toText(
        await env.exec("paste -d, file1.txt file2.txt"),
      );
      expect(result.stdout).toBe("a,1\nb,2\nc,3\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
  ...
  describe("-s (serial)", () => {
    it("should paste lines horizontally in serial mode", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste -s file1.txt"));
    ...
    it("should paste multiple files serially", async () => {
      const env = createEnv();
      const result = await toText(
        await env.exec("paste -s file1.txt file2.txt"),
      );
      expect(result.stdout).toBe("a\tb\tc\n1\t2\t3\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
    ...
    it("should use custom delimiter in serial mode", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste -s -d, file1.txt"));
      expect(result.stdout).toBe("a,b,c\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle combined -sd option", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste -sd, file1.txt"));
      expect(result.stdout).toBe("a,b,c\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("edge cases", () => {
    it("should handle empty file", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste empty.txt file1.txt"));
      expect(result.stdout).toBe("\ta\n\tb\n\tc\n");
      expect(result.stderr).toBe("");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should return error for non-existent file", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste nonexistent.txt"));
      expect(result.stdout).toBe("");
      expect(result.stderr).toBe(
    ...
    it("should return error for unknown option", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste -x file1.txt"));
      expect(result.stdout).toBe("");
    ...
    it("should return error for unknown long option", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste --unknown file1.txt"));
      expect(result.stdout).toBe("");
  ...
  describe("--help", () => {
    it("should show help", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("paste --help"));
