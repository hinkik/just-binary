...
describe("basename command", () => {
  it("should extract basename from path", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("basename /usr/bin/sort"));
  ...
  it("should handle path without directory", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("basename file.txt"));
  ...
  it("should remove suffix when provided", async () => {
    ...
    const result = await toText(
      await env.exec("basename /path/to/file.txt .txt"),
    );
  ...
  it("should handle -s suffix option", async () => {
    const env = new Bash();
    const result = await toText(
      await env.exec("basename -s .txt /path/file.txt"),
    );
    expect(result.stdout).toBe("file\n");
  });
  ...
  it("should show error for missing operand", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("basename"));
  ...
  it("should show help with --help", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("basename --help"));
