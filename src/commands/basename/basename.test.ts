...
describe("basename command", () => {
  it("should extract basename from path", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename /usr/bin/sort"));
  ...
  it("should handle path without directory", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename file.txt"));
    expect(result.stdout).toBe("file.txt\n");
  });

  it("should remove suffix when provided", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename /path/to/file.txt .txt"));
    expect(result.stdout).toBe("file\n");
  });

  it("should handle -s suffix option", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename -s .txt /path/file.txt"));
    expect(result.stdout).toBe("file\n");
  });
  ...
  it("should handle multiple files with -a", async () => {
    const env = new Bash();
    const result = toText(
      await env.exec("basename -a /path/one.txt /path/two.txt"),
    );
  ...
  it("should handle --suffix with multiple files", async () => {
    const env = new Bash();
    const result = toText(
      await env.exec("basename --suffix=.txt /path/one.txt /path/two.txt"),
    );
  ...
  it("should show error for missing operand", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename"));
  ...
  it("should show help with --help", async () => {
    const env = new Bash();
    const result = toText(await env.exec("basename --help"));
