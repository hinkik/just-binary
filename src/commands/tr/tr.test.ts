...
describe("tr command", () => {
  ...
  it("should convert lowercase to uppercase", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("echo 'hello world' | tr 'a-z' 'A-Z'"),
    );
  ...
  it("should delete specified characters", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'hello world' | tr -d 'aeiou'"));
  ...
  it("should squeeze repeated characters", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'hello    world' | tr -s ' '"));
    expect(result.stdout).toBe("hello world\n");
    expect(result.stderr).toBe("");
    expect(result.exitCode).toBe(0);
  });
  ...
  it("should translate specific characters", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'abc' | tr 'abc' 'xyz'"));
  ...
  it("should replace spaces with underscores", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'hello world' | tr ' ' '_'"));
  ...
  it("should handle character ranges", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo '12345' | tr '1-5' 'a-e'"));
  ...
  it("should delete digits", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'abc123def' | tr -d '0-9'"));
  ...
  it("should return error when missing operand", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'hello' | tr"));
  ...
  it("should return error when missing second set", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'hello' | tr 'a-z'"));
    expect(result.stdout).toBe("");
  ...
  it("should handle shorter set2", async () => {
    const env = createEnv();
    const result = toText(await env.exec("echo 'aabbcc' | tr 'abc' 'x'"));
