...
describe("tr -c (complement)", () => {
  describe("complement with delete", () => {
    it("deletes characters NOT in set1", async () => {
      ...
      const result = toText(
        await env.exec("echo 'abc123def456' | tr -cd '0-9'"),
      );
    ...
    it("keeps only alphanumeric characters", async () => {
      ...
      const result = toText(
        await env.exec("echo 'hello, world! 123' | tr -cd '[:alnum:]'"),
      );
    ...
    it("keeps only letters", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo 'a1b2c3' | tr -cd '[:alpha:]'"),
      );
    ...
    it("uses -C as equivalent to -c", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo 'abc123' | tr -Cd '0-9'"));
    ...
    it("uses --complement long option", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo 'abc123' | tr --complement -d '0-9'"),
      );
      expect(result.stdout).toBe("123");
    });
  });
  ...
  describe("complement with translate", () => {
    it("translates characters NOT in set1", async () => {
      ...
      const result = toText(
        await env.exec("echo 'abc123def' | tr -c '0-9' 'X'"),
      );
    ...
    it("replaces non-alphanumeric with dash", async () => {
      ...
      const result = toText(
        await env.exec("echo 'hello world!' | tr -c '[:alnum:]' '-'"),
      );
  ...
  describe("complement with squeeze", () => {
    it("squeezes characters NOT in set1", async () => {
      ...
      const result = toText(
        await env.exec("echo 'aaa111bbb222' | tr -cs '0-9' 'X'"),
      );
  ...
  describe("combined with other options", () => {
    it("combines -c with -d", async () => {
      ...
      const result = toText(
        await env.exec("echo 'test123test' | tr -cd '[:alpha:]'"),
      );
