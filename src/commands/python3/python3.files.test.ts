...
EOF`);
      const result = await toText(await env.exec("python3 /tmp/script.py"));
...
EOF`);
      const result = await toText(
        await env.exec("python3 /tmp/args.py foo bar baz"),
      );
...
print(sys.argv[0])
EOF`);
      const result = await toText(await env.exec("python3 /tmp/argv0.py"));
    ...
    it("should error on missing script file", async () => {
      const env = new Bash({ python: true });
      const result = await toText(
        await env.exec("python3 /tmp/nonexistent.py"),
      );
...
print(result)
EOF`);
      const result = await toText(await env.exec("python3 /tmp/multiline.py"));
...
EOF`);
      const result = await toText(await env.exec("python3 /tmp/imports.py"));
...
EOF`);
      const result = await toText(
        await env.exec("python3 /tmp/syntax_error.py"),
      );
...
EOF`);
      const result = await toText(
        await env.exec("python3 /tmp/runtime_error.py"),
      );
...
describe("python3 file I/O", () => {
  describe("file read/write", () => {
    ...
    it("should write files readable by bash", async () => {
      ...
      const result = await toText(await env.exec("cat /tmp/pyfile.txt"));
    ...
    it("should append to files", async () => {
      ...
      expect(pyResult.stderr).toBe("");
      expect(pyResult.exitCode).toBe(0);
      const result = await toText(await env.exec("cat /tmp/append.txt"));
...
"`),
    ...
    it("should create directories", async () => {
      ...
      expect(pyResult.stderr).toBe("");
      expect(pyResult.exitCode).toBe(0);
      const result = await toText(await env.exec("ls -d /tmp/newdir/subdir"));
...
EOF`);
      const result = await toText(await env.exec("python3 /tmp/main.py"));
      expect(result.stderr).toBe("");
      expect(result.stdout).toBe("Hello, World!\n");
      expect(result.exitCode).toBe(0);
    });
