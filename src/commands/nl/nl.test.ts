...
describe("nl", () => {
  ...
  it("numbers lines from file", async () => {
    ...
    const result = toText(await bash.exec("nl /test.txt"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles empty input", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf '' | nl"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles single line without newline", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'hello' | nl"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles multiple files", async () => {
    ...
    const result = toText(await bash.exec("nl /a.txt /b.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "     1\tone\n     2\ttwo\n     3\tthree\n     4\tfour\n",
    );
  ...
  it("continues numbering across files", async () => {
    ...
    const result = toText(await bash.exec("nl -v 10 /a.txt /b.txt"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles file not found", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("nl /nonexistent.txt"));
  ...
  it("shows help with --help", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("nl --help"));
    expect(result.exitCode).toBe(0);
  ...
  it("errors on invalid body style", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | nl -b x"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on invalid number format", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | nl -n xx"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on invalid width", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | nl -w abc"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown short flag", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | nl -x"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown long flag", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | nl --unknown"));
    expect(result.exitCode).toBe(1);
