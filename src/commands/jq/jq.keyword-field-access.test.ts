...
describe("jq keyword field access", () => {
  describe("field names that are keywords should be accessible with dot notation", () => {
    it("should access .label field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"label\":\"hello\"}' | jq '.label'"),
      );
    ...
    it("should access .and field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"and\":true}' | jq '.and'"),
      );
    ...
    it("should access .or field", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo '{\"or\":false}' | jq '.or'"));
    ...
    it("should access .not field", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo '{\"not\":42}' | jq '.not'"));
    ...
    it("should access .if field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"if\":\"value\"}' | jq '.if'"),
      );
    ...
    it("should access .try field", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo '{\"try\":1}' | jq '.try'"));
    ...
    it("should access .catch field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"catch\":2}' | jq '.catch'"),
      );
    ...
    it("should access .reduce field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"reduce\":\"data\"}' | jq '.reduce'"),
      );
    ...
    it("should access .foreach field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"foreach\":\"items\"}' | jq '.foreach'"),
      );
    ...
    it("should access .def field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"def\":\"definition\"}' | jq '.def'"),
      );
    ...
    it("should access .break field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"break\":\"stop\"}' | jq '.break'"),
      );
  ...
  describe("keyword as object key", () => {
    ...
    it("should allow keyword as object shorthand key", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"label\":\"hello\"}' | jq -c '{label}'"),
      );
      expect(result.stdout).toBe('{"label":"hello"}\n');
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should allow 'not' as object key", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"not\":true}' | jq -c '{not: .not}'"),
      );
    ...
    it("should allow 'and' as object shorthand key", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"and\":1}' | jq -c '{and}'"),
      );
  ...
  describe("keyword as object destructuring pattern key", () => {
    ...
    it("should allow keyword shorthand in object destructuring", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("echo '{\"not\":42}' | jq '. as {$not} | $not'"),
      );
      expect(result.stdout).toBe("42\n");
      expect(result.exitCode).toBe(0);
    });
