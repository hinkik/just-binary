...
describe("jq builtin functions", () => {
  ...
  describe("length", () => {
    it("should get length of array", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[1,2,3,4,5]' | jq 'length'"),
      );
    ...
    it("should get length of string", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '\"hello\"' | jq 'length'"),
      );
      expect(result.stdout).toBe("5\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("type", () => {
    it("should get type of object", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '{\"a\":1}' | jq 'type'"),
      );
    ...
    it("should get type of array", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("echo '[1,2]' | jq 'type'"));
  ...
  describe("first and last", () => {
    it("should get first element", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[5,10,15]' | jq 'first'"),
      );
      expect(result.stdout).toBe("5\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should get last element", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[5,10,15]' | jq 'last'"),
      );
    ...
    it("should get first of expression", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'first(range(10))'"));
  ...
  describe("reverse", () => {
    it("should reverse array", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[1,2,3]' | jq 'reverse'"),
      );
      expect(result.stdout).toBe("[\n  3,\n  2,\n  1\n]\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("sort and unique", () => {
    it("should sort array", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("echo '[3,1,2]' | jq 'sort'"));
      expect(result.stdout).toBe("[\n  1,\n  2,\n  3\n]\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should get unique values", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[1,2,1,3,2]' | jq 'unique'"),
      );
      expect(result.stdout).toBe("[\n  1,\n  2,\n  3\n]\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("add", () => {
    it("should add numbers", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[1,2,3,4]' | jq 'add'"),
      );
  ...
  describe("min and max", () => {
    it("should get min value", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[5,2,8,1]' | jq 'min'"),
      );
    ...
    it("should get max value", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo '[5,2,8,1]' | jq 'max'"),
      );
  ...
  describe("range", () => {
    it("should generate range", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n '[range(5)]'"));
      expect(result.stdout).toBe("[\n  0,\n  1,\n  2,\n  3,\n  4\n]\n");
    });

    it("should generate range with start and end", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n '[range(2;5)]'"));
      expect(result.stdout).toBe("[\n  2,\n  3,\n  4\n]\n");
    });
  });
  ...
  describe("limit", () => {
    it("should limit results", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("jq -n '[limit(3; range(10))]'"),
      );
      expect(result.stdout).toBe("[\n  0,\n  1,\n  2\n]\n");
    });
  });
  ...
  describe("math functions with two arguments", () => {
    it("should compute pow(base; exp)", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'pow(2; 3)'"));
      expect(result.stdout).toBe("8\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should compute pow with non-integer exponent", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'pow(4; 0.5)'"));
      expect(result.stdout).toBe("2\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should compute atan2(y; x)", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'atan2(3; 4)'"));
    ...
    it("should compute atan2 with negative values", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'atan2(-1; -1)'"));
    ...
    it("should return null for pow with non-numeric args", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'pow(\"a\"; 2)'"));
      expect(result.stdout).toBe("null\n");
    });

    it("should return null for atan2 with non-numeric args", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("jq -n 'atan2(\"a\"; 2)'"));
      expect(result.stdout).toBe("null\n");
    });
  });
});
