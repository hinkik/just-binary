...
describe("awk escape sequences", () => {
  ...
  it("should print NF=0 for empty line", async () => {
    const env = new Bash();
    const result = await toText(await env.exec(`echo '' | awk '{ print NF }'`));
  ...
  it("should print NF=0 for tab-only line", async () => {
    const env = new Bash();
    const result = await toText(
      await env.exec(`echo $'\\t' | awk '{ print NF }'`),
    );
    expect(result.stdout).toBe("0\n");
    expect(result.exitCode).toBe(0);
  });
...
describe("awk command", () => {
  describe("basic field access", () => {
    it("should print entire line with $0", async () => {
      ...
      const result = await toText(await env.exec("awk '{print $0}' /data.txt"));
    ...
    it("should print first field with $1", async () => {
      ...
      const result = await toText(await env.exec("awk '{print $1}' /data.txt"));
    ...
    it("should print multiple fields", async () => {
      ...
      const result = await toText(
        await env.exec("awk '{print $1, $3}' /data.txt"),
      );
    ...
    it("should handle missing fields gracefully", async () => {
      ...
      const result = await toText(await env.exec("awk '{print $2}' /data.txt"));
  ...
  describe("field separator -F", () => {
    it("should use custom field separator", async () => {
      ...
      const result = await toText(
        await env.exec("awk -F',' '{print $2}' /data.csv"),
      );
  ...
  describe("built-in variables", () => {
    it("should track NR (record number)", async () => {
      ...
      const result = await toText(
        await env.exec("awk '{print NR, $0}' /data.txt"),
      );
    ...
    it("should track NF (number of fields)", async () => {
      ...
      const result = await toText(await env.exec("awk '{print NF}' /data.txt"));
  ...
  describe("pattern matching", () => {
    it("should filter lines with regex pattern", async () => {
      ...
      const result = await toText(
        await env.exec("awk '/^a/{print}' /data.txt"),
      );
    ...
    it("should match with NR condition", async () => {
      ...
      const result = await toText(
        await env.exec("awk 'NR==2{print}' /data.txt"),
      );
    ...
    it("should match with NR > condition", async () => {
      ...
      const result = await toText(
        await env.exec("awk 'NR>1{print}' /data.txt"),
      );
      expect(result.stdout).toBe("line2\nline3\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("stdin input", () => {
    it("should read from piped stdin", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("echo 'a b c' | awk '{print $2}'"),
      );
      expect(result.stdout).toBe("b\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("error handling", () => {
    it("should error on missing program", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("awk"));
    ...
    it("should error on missing file", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("awk '{print}' /nonexistent.txt"),
      );
      expect(result.exitCode).toBe(1);
    ...
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("awk --help"));
  ...
  describe("variable comparisons in conditions", () => {
    ...
    it("should handle NF comparisons", async () => {
      ...
      const result = await toText(
        await env.exec("awk 'NF>1{print}' /data.txt"),
      );
  ...
  describe("power operator", () => {
    it("should compute power with ^", async () => {
      ...
      const result = await toText(
        await env.exec("awk '{print 2^3}' /data.txt"),
      );
      expect(result.stdout).toBe("8\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should compute power with **", async () => {
      ...
      const result = await toText(
        await env.exec("awk '{print 3**2}' /data.txt"),
      );
  ...
  describe("exit and next statements", () => {
    it("should exit with code", async () => {
      ...
      const result = await toText(
        await env.exec("awk 'NR==2{exit 5}' /data.txt"),
      );
    ...
    it("should skip to next line with next", async () => {
      ...
      const result = await toText(
        await env.exec("awk '/b/{next}{print}' /data.txt"),
      );
      expect(result.stdout).toBe("a\nc\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("printf formats", () => {
    ...
    it("should format octal with %o", async () => {
      ...
      const result = await toText(
        await env.exec("awk 'BEGIN{printf \"%o\\n\", 8}'"),
      );
      expect(result.stdout).toBe("10\n");
      expect(result.exitCode).toBe(0);
    });
