...
describe("awk field handling", () => {
  describe("field access", () => {
    it("should access $0 as entire line", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c" | awk '{ print $0 }'`),
      );
    ...
    it("should access individual fields", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c d e" | awk '{ print $1, $3, $5 }'`),
      );
    ...
    it("should return empty for non-existent field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b" | awk '{ print "[" $10 "]" }'`),
      );
    ...
    it("should access last field with $NF", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "first second last" | awk '{ print $NF }'`),
      );
    ...
    it("should access second-to-last with $(NF-1)", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c d" | awk '{ print $(NF-1) }'`),
      );
    ...
    it("should use variable for field index", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c d" | awk '{ i=3; print $i }'`),
      );
      expect(result.stdout).toBe("c\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should use expression for field index", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c d" | awk '{ print $(1+2) }'`),
      );
      expect(result.stdout).toBe("c\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("field modification", () => {
    it("should modify specific field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c" | awk '{ $2 = "X"; print }'`),
      );
    ...
    it("should update $0 when field is modified", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "one two three" | awk '{ $2 = "TWO"; print $0 }'`),
      );
    ...
    it("should extend fields when assigning beyond NF", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b" | awk '{ $5 = "e"; print NF, $0 }'`),
      );
    ...
    it("should modify first field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "hello world" | awk '{ $1 = "HELLO"; print }'`),
      );
    ...
    it("should modify last field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c" | awk '{ $NF = "C"; print }'`),
      );
      expect(result.stdout).toBe("a b C\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("NF (Number of Fields)", () => {
    it("should report correct number of fields", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c d e" | awk '{ print NF }'`),
      );
    ...
    it("should report 1 for single field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "hello" | awk '{ print NF }'`),
      );
    ...
    it("should update when field is added", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b" | awk '{ $4 = "d"; print NF }'`),
      );
  ...
  describe("OFS (Output Field Separator)", () => {
    ...
    it("should change OFS mid-program", async () => {
      ...
      const result = toText(
        await env.exec(`awk 'NR==1{OFS=","} { $1=$1; print }' /data.txt`),
      );
  ...
  describe("ORS (Output Record Separator)", () => {
    it("should use ORS after each print", async () => {
      ...
      const result = toText(
        await env.exec(`awk 'BEGIN{ORS=";"} { print }' /data.txt`),
      );
  ...
  describe("FS (Field Separator)", () => {
    ...
    it("should use -F option for field separator", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a:b:c" | awk -F: '{ print $2 }'`),
      );
    ...
    it("should set FS in BEGIN block", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a,b,c" | awk 'BEGIN{FS=","} { print $2 }'`),
      );
      expect(result.stdout).toBe("b\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle tab as separator", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`printf "a\\tb\\tc" | awk -F'\\t' '{ print $2 }'`),
      );
      expect(result.stdout).toBe("b\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle empty fields with single char FS", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a::c" | awk -F: '{ print NF, "[" $2 "]" }'`),
      );
  ...
  describe("field assignment with arithmetic", () => {
    it("should perform arithmetic on fields", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "10 20 30" | awk '{ print $1 + $2 + $3 }'`),
      );
    ...
    it("should multiply fields", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "5 6" | awk '{ print $1 * $2 }'`),
      );
    ...
    it("should modify field with arithmetic", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "10 20" | awk '{ $1 = $1 * 2; print }'`),
      );
    ...
    it("should add new computed field", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "10 20" | awk '{ $(NF+1) = $1 + $2; print }'`),
      );
  ...
  describe("special field cases", () => {
    ...
    it("should handle leading whitespace", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "   a b c" | awk '{ print NF, $1 }'`),
      );
    ...
    it("should handle trailing whitespace", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c   " | awk '{ print NF, $NF }'`),
      );
    ...
    it("should handle multiple consecutive delimiters", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a     b     c" | awk '{ print NF }'`),
      );
      expect(result.stdout).toBe("3\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("field-based filtering", () => {
    it("should filter by field value", async () => {
      ...
      const result = toText(
        await env.exec(`awk '$2 == 25 { print $1 }' /data.txt`),
      );
    ...
    it("should filter by field regex", async () => {
      ...
      const result = toText(
        await env.exec(`awk '$1 ~ /^a/ { print }' /data.txt`),
      );
    ...
    it("should combine field conditions", async () => {
      ...
      const result = toText(
        await env.exec(`awk '$2 == 10 && $3 == "x" { print $1 }' /data.txt`),
      );
      expect(result.stdout).toBe("a\n");
      expect(result.exitCode).toBe(0);
    });
