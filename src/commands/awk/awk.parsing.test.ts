...
describe("awk parsing", () => {
  ...
  describe("string parsing", () => {
    it("should handle escaped quotes in string", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print "he said \\"hello\\"" }'`),
      );
    ...
    it("should handle backslash sequences", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print "a\\tb\\nc" }'`),
      );
  ...
  describe("number parsing", () => {
    it("should parse integers", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 42 }'`),
      );
    ...
    it("should parse floating point", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 3.14 }'`),
      );
    ...
    it("should parse scientific notation", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 1e3 }'`),
      );
    ...
    it("should parse negative scientific notation", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 1e-2 }'`),
      );
    ...
    it("should parse leading decimal", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print .5 }'`),
      );
  ...
  describe("regex parsing", () => {
    it("should parse simple regex", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "hello" | awk '/hello/ { print "matched" }'`),
      );
    ...
    it("should parse regex with brackets", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "abc" | awk '/[abc]+/ { print "matched" }'`),
      );
      expect(result.stdout).toBe("matched\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should parse regex with quantifiers", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "aaa" | awk '/a+/ { print "matched" }'`),
      );
      expect(result.stdout).toBe("matched\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should distinguish regex from division", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { x = 10 / 2; print x }'`),
      );
  ...
  describe("operator parsing", () => {
    ...
    it("should parse increment/decrement", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { x=5; print ++x, x++, x }'`),
      );
  ...
  describe("block structure parsing", () => {
    it("should parse empty blocks", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "test" | awk '{ } { print "ok" }'`),
      );
    ...
    it("should parse pattern without action", async () => {
      const env = new Bash();
      const result = toText(await env.exec(`echo "hello" | awk '/hello/'`));
    ...
    it("should parse action without pattern", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "hello" | awk '{ print "matched" }'`),
      );
      expect(result.stdout).toBe("matched\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("statement parsing", () => {
    it("should parse if statement", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { if (1) print "yes" }'`),
      );
  ...
  describe("expression parsing edge cases", () => {
    ...
    it("should parse chained field access", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "1 2 3" | awk '{ print $($1) }'`),
      );
      // $1 = "1", $($1) = $(1) = "1"
      expect(result.stdout).toBe("1\n");
      expect(result.exitCode).toBe(0);
    ...
    it("should parse array with expression index", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { a[1+1] = "two"; print a[2] }'`),
      );
  ...
  describe("error cases", () => {
    ...
    it("should error on invalid option", async () => {
      const env = new Bash();
      const result = toText(await env.exec(`awk --invalid-option '{print}'`));
      expect(result.exitCode).toBe(1);
    });
