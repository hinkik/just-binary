...
describe("awk edge cases", () => {
  describe("empty input handling", () => {
    it("should handle empty file", async () => {
      ...
      const result = toText(
        await env.exec(`awk '{ print "line" }' /empty.txt`),
      );
    ...
    it("should handle empty stdin", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo -n "" | awk '{ print "line" }'`),
      );
      expect(result.stdout).toBe("");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle file with only newlines", async () => {
      ...
      const result = toText(
        await env.exec(`awk '{ print NR, NF }' /newlines.txt`),
      );
  ...
  describe("special characters in data", () => {
    it("should handle quotes in data", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo 'he said "hello"' | awk '{ print $3 }'`),
      );
    ...
    it("should handle backslash in data", async () => {
      ...
      const result = toText(
        await env.exec(`echo 'path\\\\to\\\\file' | awk '{ print }'`),
      );
    ...
    it("should handle brackets in data", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "[test]" | awk '{ print $1 }'`),
      );
    ...
    it("should handle dollar sign in data", async () => {
      ...
      const result = toText(
        await env.exec(`echo 'price: \\$100' | awk '{ print $2 }'`),
      );
    ...
    it("should handle ampersand in data", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a & b" | awk '{ print $2 }'`),
      );
      expect(result.stdout).toBe("&\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("numeric edge cases", () => {
    it("should handle very large numbers", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 1e308 }'`),
      );
    ...
    it("should handle very small numbers", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 1e-308 }'`),
      );
      expect(result.stdout).toMatch(/1e-308/i);
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle negative zero", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print -0 }'`),
      );
    ...
    it("should handle floating point precision", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print 0.1 + 0.2 }'`),
      );
  ...
  describe("string edge cases", () => {
    it("should handle empty string comparison", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print ("" == "") }'`),
      );
    ...
    it("should handle null character equivalent", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { x = ""; print length(x) }'`),
      );
      expect(result.stdout).toBe("0\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("regex edge cases", () => {
    it("should match empty regex", async () => {
      const env = new Bash();
      const result = toText(await env.exec(`echo "test" | awk '//' | head -1`));
      expect(result.stdout).toBe("test\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle regex with special chars", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a.b" | awk '/a\\.b/ { print "matched" }'`),
      );
    ...
    it("should handle regex at line start", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "test" | awk '/^test/ { print "yes" }'`),
      );
    ...
    it("should handle case-sensitive regex", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "TEST" | awk '/test/ { print "matched" }'`),
      );
      expect(result.stdout).toBe("");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("control flow edge cases", () => {
    it("should handle empty action block", async () => {
      const env = new Bash();
      const result = toText(await env.exec(`echo "test" | awk '{ }'`));
      expect(result.stdout).toBe("");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle multiple semicolons", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { x=1;; y=2;;; print x+y }'`),
      );
  ...
  describe("variable edge cases", () => {
    it("should handle uninitialized variable as number", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print x + 0 }'`),
      );
      expect(result.stdout).toBe("0\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle uninitialized variable as string", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print "[" x "]" }'`),
      );
      expect(result.stdout).toBe("[]\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle variable shadowing built-in", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "a b c" | awk '{ NF = 100; print NF }'`),
      );
      expect(result.stdout).toBe("100\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle assignment in condition", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { if (x = 5) print x }'`),
      );
  ...
  describe("array edge cases", () => {
    ...
    it("should handle delete on empty array", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { delete arr["x"]; print "ok" }'`),
      );
  ...
  describe("print edge cases", () => {
    it("should handle print without arguments", async () => {
      const env = new Bash();
      const result = toText(await env.exec(`echo "test" | awk '{ print }'`));
      expect(result.stdout).toBe("test\n");
      expect(result.exitCode).toBe(0);
    });
  ...
  describe("BEGIN/END edge cases", () => {
    ...
    it("should preserve variables from BEGIN in main", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "test" | awk 'BEGIN { x = 42 } { print x }'`),
      );
    ...
    it("should access NR in END", async () => {
      ...
      const result = toText(
        await env.exec(`awk 'END { print "Total lines:", NR }' /data.txt`),
      );
  ...
  describe("type coercion edge cases", () => {
    it("should compare string and number", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print ("10" == 10) }'`),
      );
      expect(result.stdout).toBe("1\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should handle number in string context", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec(`echo "" | awk 'BEGIN { print length(12345) }'`),
      );
      expect(result.stdout).toBe("5\n");
      expect(result.exitCode).toBe(0);
    });
