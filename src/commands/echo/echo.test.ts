...
describe("echo", () => {
  ...
  it("should echo multiple arguments", async () => {
    const env = new Bash();
    const result = toText(await env.exec("echo one two three"));
    expect(result.stdout).toBe("one two three\n");
  });

  it("should handle -n flag (no newline)", async () => {
    const env = new Bash();
    const result = toText(await env.exec("echo -n hello"));
  ...
  it("should handle -e flag with \\r (carriage return)", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo -e "hello\\rworld"'));
    expect(result.stdout).toBe("hello\rworld\n");
  });

  it("should handle -e flag with multiple escape sequences", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo -e "line1\\nline2\\ttabbed"'));
    expect(result.stdout).toBe("line1\nline2\ttabbed\n");
  });

  it("should handle combined -en flags", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo -en "hello\\nworld"'));
    expect(result.stdout).toBe("hello\nworld");
  });

  it("should handle combined -ne flags", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo -ne "a\\tb"'));
  ...
  it("should echo text starting with dash when using --", async () => {
    const env = new Bash();
    const result = toText(await env.exec("echo -- -n"));
