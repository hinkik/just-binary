...
describe("stat command", () => {
  describe("basic usage", () => {
    it("should display file info", async () => {
      ...
      const result = await toText(await env.exec("stat /test.txt"));
    ...
    it("should display directory info", async () => {
      ...
      const result = await toText(await env.exec("stat /mydir"));
    ...
    it("should error on missing file", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("stat /nonexistent"));
    ...
    it("should error without operand", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("stat"));
  ...
  describe("-c option (format)", () => {
    it("should format with %n (filename)", async () => {
      ...
      const result = await toText(await env.exec('stat -c "%n" /test.txt'));
    ...
    it("should format with %s (size)", async () => {
      ...
      const result = await toText(await env.exec('stat -c "%s" /test.txt'));
    ...
    it("should format with %F (file type)", async () => {
      ...
      const fileResult = await toText(
        await env.exec('stat -c "%F" /mydir/file.txt'),
      );
      expect(fileResult.stdout.trim()).toBe("regular file");

      const dirResult = await toText(await env.exec('stat -c "%F" /mydir'));
    ...
    it("should format with combined specifiers", async () => {
      ...
      const result = await toText(
        await env.exec('stat -c "%n: %s bytes" /test.txt'),
      );
  ...
  describe("multiple files", () => {
    it("should stat multiple files", async () => {
      ...
      const result = await toText(await env.exec("stat /a.txt /b.txt"));
    ...
    it("should continue on error", async () => {
      ...
      const result = await toText(
        await env.exec("stat /exists.txt /missing.txt"),
      );
  ...
  describe("help option", () => {
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("stat --help"));
