...
describe('cut command', () => {
  const createEnv = () =>
    new BashEnv({
      files: {
        '/test/passwd.txt': 'root:x:0:0:root:/root:/bin/bash\nuser:x:1000:1000:User:/home/user:/bin/zsh\n',
  ...
  it('should handle field from end with open range', async () => {
    ...
    expect(result.stdout).toBe('root:/root:/bin/bash\nUser:/home/user:/bin/zsh\n');
    expect(result.stderr).toBe('');
    expect(result.exitCode).toBe(0);
  });
  ...
  it('should return error for non-existent file', async () => {
    ...
    expect(result.stderr).toBe('cut: /test/nonexistent.txt: No such file or directory\n');
  ...
  it('should return error when no field or char specified', async () => {
    ...
    expect(result.stdout).toBe('');
    expect(result.stderr).toBe('cut: you must specify a list of bytes, characters, or fields\n');
    expect(result.exitCode).toBe(1);
  });
});
