...
describe("true", () => {
  ...
  it("should ignore all arguments", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("true --help -x --anything"));
    expect(result.exitCode).toBe(0);
  });

  it("should work in conditionals", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("true && echo yes || echo no"));
...
describe("false", () => {
  ...
  it("should ignore all arguments", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("false --help -x --anything"));
    expect(result.exitCode).toBe(1);
  });

  it("should work in conditionals", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("false && echo yes || echo no"));
