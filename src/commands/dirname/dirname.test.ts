...
describe("dirname command", () => {
  it("should extract directory from path", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("dirname /usr/bin/sort"));
  ...
  it("should return . for file without directory", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("dirname file.txt"));
  ...
  it("should handle multiple paths", async () => {
    ...
    const result = await toText(
      await env.exec("dirname /path/to/file1 /another/path/file2"),
    );
  ...
  it("should show error for missing operand", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("dirname"));
  ...
  it("should show help with --help", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("dirname --help"));
