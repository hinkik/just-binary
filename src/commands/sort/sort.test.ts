...
describe("sort command", () => {
  ...
  it("should sort lines alphabetically", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort /test/names.txt"));
  ...
  it("should sort lines in reverse order with -r", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort -r /test/names.txt"));
  ...
  it("should sort numerically with -n", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort -n /test/numbers.txt"));
  ...
  it("should sort numerically in reverse with -rn", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort -rn /test/numbers.txt"));
  ...
  it("should remove duplicates with -u", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort -u /test/duplicates.txt"));
  ...
  it("should sort by key field with -k", async () => {
    ...
    const result = await toText(
      await env.exec("sort -k2 -n /test/columns.txt"),
    );
  ...
  it("should read from stdin via pipe", async () => {
    const env = createEnv();
    const result = await toText(await env.exec('echo -e "c\\nb\\na" | sort'));
  ...
  it("should handle case-sensitive sorting", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort /test/mixed.txt"));
  ...
  it("should return error for non-existent file", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort /test/nonexistent.txt"));
  ...
  it("should handle empty input", async () => {
    const env = createEnv();
    const result = await toText(await env.exec('echo "" | sort'));
    expect(result.stdout).toBe("\n");
    expect(result.stderr).toBe("");
    expect(result.exitCode).toBe(0);
  });
  ...
  it("should handle combined flags -nr", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("sort -nr /test/numbers.txt"));
    expect(result.stdout).toBe("20\n10\n5\n2\n1\n");
    expect(result.stderr).toBe("");
    expect(result.exitCode).toBe(0);
  });
  ...
  describe("-f flag (case-insensitive)", () => {
    it("should sort case-insensitively with -f", async () => {
      const env = createEnv();
      const result = await toText(await env.exec("sort -f /test/mixed.txt"));
    ...
    it("should sort case-insensitively with --ignore-case", async () => {
      ...
      const result = await toText(
        await env.exec("sort --ignore-case /test.txt"),
      );
      expect(result.stdout).toBe("apple\nBanana\nCherry\n");
      expect(result.exitCode).toBe(0);
    });
    ...
    it("should combine -f with -r for reverse case-insensitive", async () => {
      ...
      const result = await toText(await env.exec("sort -fr /test.txt"));
    ...
    it("should combine -f with -u for unique case-insensitive", async () => {
      ...
      const result = await toText(await env.exec("sort -fu /test.txt"));
    ...
    it("should work with -k field and -f", async () => {
      ...
      const result = await toText(await env.exec("sort -f -k2 /test.txt"));
    ...
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("sort --help"));
      expect(result.exitCode).toBe(0);
  ...
  describe("complex -k syntax", () => {
    it("should sort by field range -k1,2", async () => {
      ...
      const result = await toText(await env.exec("sort -k1,2 /test.txt"));
    ...
    it("should sort by single field only with -k2,2", async () => {
      ...
      const result = await toText(await env.exec("sort -k2,2 /test.txt"));
      expect(result.stdout).toBe("2 apple\n1 banana\n3 cherry\n");
    });
    ...
    it("should support per-key numeric modifier -k2n", async () => {
      ...
      const result = await toText(await env.exec("sort -k2n /test.txt"));
    ...
    it("should support per-key reverse modifier -k1r", async () => {
      ...
      const result = await toText(await env.exec("sort -k1r /test.txt"));
    ...
    it("should support combined modifiers -k2,2nr", async () => {
      ...
      const result = await toText(await env.exec("sort -k2,2nr /test.txt"));
    ...
    it("should support multiple keys for secondary sort", async () => {
      ...
      const result = await toText(
        await env.exec("sort -k1,1 -k2,2n /test.txt"),
      );
    ...
    it("should support per-key ignore-case -k1f", async () => {
      ...
      const result = await toText(await env.exec("sort -k1f /test.txt"));
      expect(result.stdout).toBe("apple\nBANANA\nZebra\n");
    });
    ...
    it("should support custom delimiter with -t", async () => {
      ...
      const result = await toText(await env.exec("sort -t: -k2n /test.txt"));
      expect(result.stdout).toBe("a:1\nb:2\nc:3\n");
    });
    ...
    it("should handle --key= syntax", async () => {
      ...
      const result = await toText(await env.exec("sort --key=1n /test.txt"));
