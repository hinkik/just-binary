...
describe("xan count", () => {
  it("counts rows excluding header", async () => {
    ...
    const result = await toText(await bash.exec("xan count /users.csv"));
  ...
  it("counts rows in numbers.csv", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan count /numbers.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("returns 0 for empty file", async () => {
    ...
    const result = await toText(await bash.exec("xan count /empty.csv"));
    expect(result.exitCode).toBe(0);
...
describe("xan headers", () => {
  it("lists column names with indices", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan headers /users.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("lists just names with -j", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan headers -j /users.csv"));
    expect(result.exitCode).toBe(0);
...
describe("xan head", () => {
  it("shows first 10 rows by default", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan head /users.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("shows first N rows with -l", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan head -l 2 /users.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "name,age,email,active\nalice,30,alice@example.com,true\nbob,25,bob@example.com,false\n",
    );
...
describe("xan tail", () => {
  it("shows last 10 rows by default", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan tail /users.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "name,age,email,active\nalice,30,alice@example.com,true\nbob,25,bob@example.com,false\ncharlie,35,charlie@example.com,true\ndiana,28,diana@example.com,true\n",
    );
  ...
  it("shows last N rows with -l", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan tail -l 2 /users.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "name,age,email,active\ncharlie,35,charlie@example.com,true\ndiana,28,diana@example.com,true\n",
    );
...
describe("xan slice", () => {
  it("extracts rows with -s and -e", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(
      await bash.exec("xan slice -s 1 -e 3 /users.csv"),
    );
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "name,age,email,active\nbob,25,bob@example.com,false\ncharlie,35,charlie@example.com,true\n",
  ...
  it("extracts from start with -l", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan slice -l 2 /users.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      "name,age,email,active\nalice,30,alice@example.com,true\nbob,25,bob@example.com,false\n",
    );
...
describe("xan reverse", () => {
  it("reverses row order", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan reverse /numbers.csv"));
    expect(result.exitCode).toBe(0);
...
describe("xan enum", () => {
  it("adds index column", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(await bash.exec("xan enum /numbers.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("uses custom column name with -c", async () => {
    const bash = new Bash({ files: fixtures });
    const result = await toText(
      await bash.exec("xan enum -c row_num /numbers.csv"),
    );
    expect(result.exitCode).toBe(0);
...
describe("xan error handling", () => {
  it("errors on missing file", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("xan count /nonexistent.csv"));
  ...
  it("shows help with --help", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("xan --help"));
    expect(result.exitCode).toBe(0);
  ...
  it("errors on unknown subcommand", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("xan foobar /data.csv"));
    expect(result.exitCode).toBe(1);
  ...
  it("returns proper error for unimplemented commands", async () => {
    const bash = new Bash({});
    const result = await toText(await bash.exec("xan parallel"));
    expect(result.exitCode).toBe(1);
...
describe("xan behead", () => {
  it("removes header row completely", async () => {
    ...
    const result = await toText(await bash.exec("xan behead /data.csv"));
    expect(result.exitCode).toBe(0);
...
describe("xan sample", () => {
  it("samples N rows (positional argument)", async () => {
    ...
    const result = await toText(
      await bash.exec("xan sample 3 --seed 42 /data.csv"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("returns all rows if sample size exceeds data", async () => {
    ...
    const result = await toText(await bash.exec("xan sample 10 /data.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("n\n1\n2\n3\n");
  });
  ...
  it("errors without sample size", async () => {
    ...
    const result = await toText(await bash.exec("xan sample /data.csv"));
    expect(result.exitCode).toBe(1);
...
describe("xan flatten", () => {
  ...
  it("displays records vertically", async () => {
    ...
    const result = await toText(await bash.exec("xan flatten /data.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(
      `Row n°0\n${sep}\nname alice\nage  30\n\nRow n°1\n${sep}\nname bob\nage  25\n`,
    );
  ...
  it("limits rows with -l", async () => {
    ...
    const result = await toText(await bash.exec("xan flatten -l 1 /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("works with alias f", async () => {
    ...
    const result = await toText(await bash.exec("xan f /data.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(`Row n°0\n${sep}\nx 1\n`);
  });
});
