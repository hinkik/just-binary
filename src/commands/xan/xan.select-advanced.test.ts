...
describe("xan select glob patterns", () => {
  ...
  it("selects columns by prefix glob", async () => {
    ...
    const result = await toText(
      await bash.exec("xan select 'vec_*' /data.csv"),
    );
  ...
  it("selects columns by suffix glob", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select '*_1' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("combines glob with regular column", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(
      await bash.exec("xan select 'name,vec_*' /data.csv"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("selects all with *", async () => {
    ...
    const result = await toText(await bash.exec("xan select '*' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("combines * with additional column", async () => {
    ...
    const result = await toText(await bash.exec("xan select '*,a' /data.csv"));
    expect(result.exitCode).toBe(0);
...
describe("xan select column ranges", () => {
  ...
  it("selects column range a:c", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select 'a:c' /data.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("a,b,c\n1,2,3\n");
  });
  ...
  it("selects reverse range c:a", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select 'c:a' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("selects range to end d:", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select 'd:' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("selects range from start :b", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select ':b' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("combines ranges", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(
      await bash.exec("xan select 'a:b,d:e' /data.csv"),
    );
    expect(result.exitCode).toBe(0);
...
describe("xan select negation", () => {
  ...
  it("excludes multiple columns", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(
      await bash.exec("xan select '*,!b,!d' /data.csv"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("excludes range", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(
      await bash.exec("xan select '*,!b:c' /data.csv"),
    );
    expect(result.exitCode).toBe(0);
...
describe("xan select numeric indices and ranges", () => {
  ...
  it("selects by numeric index", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select '0,2' /data.csv"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("a,c\n1,3\n");
  });
  ...
  it("selects by numeric range", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select '1-3' /data.csv"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles duplicate selections", async () => {
    const bash = new Bash({ files: { "/data.csv": DATA } });
    const result = await toText(await bash.exec("xan select 'a,a' /data.csv"));
    expect(result.exitCode).toBe(0);
