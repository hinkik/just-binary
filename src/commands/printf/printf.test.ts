...
describe("printf", () => {
  describe("basic format specifiers", () => {
    it("should format string with %s", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "Hello %s" world'));
    ...
    it("should format integer with %d", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "Number: %d" 42'));
    ...
    it("should format float with %f", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "Value: %f" 3.14'));
    ...
    it("should format hex with %x", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "Hex: %x" 255'));
    ...
    it("should format octal with %o", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "Octal: %o" 8'));
    ...
    it("should handle literal %% ", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "100%%"'));
    ...
    it("should handle multiple arguments", async () => {
      ...
      const result = await toText(
        await env.exec('printf "%s is %d years old" Alice 30'),
      );
  ...
  describe("escape sequences", () => {
    it("should handle newline \\n", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "line1\\nline2"'));
      expect(result.stdout).toBe("line1\nline2");
    });

    it("should handle tab \\t", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "col1\\tcol2"'));
  ...
  describe("width and precision", () => {
    it("should handle width specifier", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "%10s" "hi"'));
      expect(result.stdout).toBe("        hi");
    });

    it("should handle precision for floats", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "%.2f" 3.14159'));
      expect(result.stdout).toBe("3.14");
    });

    it("should handle zero-padding", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "%05d" 42'));
  ...
  describe("error handling", () => {
    ...
    it("should handle missing arguments gracefully", async () => {
      const env = new Bash();
      const result = await toText(await env.exec('printf "%s %s" only'));
    ...
    it("should handle non-numeric for %d", async () => {
      ...
      const env = new Bash();
      const result = await toText(await env.exec('printf "%d" notanumber'));
  ...
  describe("--help", () => {
    it("should display help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("printf --help"));
