...
describe("mv", () => {
  it("should move file", async () => {
    ...
    const result = await toText(await env.exec("mv /old.txt /new.txt"));
  ...
  it("should remove source after move", async () => {
    ...
    const cat = await toText(await env.exec("cat /old.txt"));
  ...
  it("should error when moving multiple files to non-directory", async () => {
    ...
    const result = await toText(
      await env.exec("mv /a.txt /b.txt /nonexistent"),
    );
  ...
  it("should move directory", async () => {
    ...
    expect(content).toBe("content");
    const ls = await toText(await env.exec("ls /srcdir"));
  ...
  it("should error on missing source", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("mv /missing.txt /dst.txt"));
    expect(result.exitCode).toBe(1);
  ...
  it("should error with missing destination", async () => {
    ...
    const result = await toText(await env.exec("mv /src.txt"));
    expect(result.exitCode).toBe(1);
  ...
  describe("flags", () => {
    it("should accept -f flag (force)", async () => {
      ...
      const result = await toText(await env.exec("mv -f /src.txt /dst.txt"));
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe("");
      const content = await env.readFile("/dst.txt");
      expect(content).toBe("new");
    ...
    it("should skip existing file with -n flag (no-clobber)", async () => {
      ...
      const result = await toText(await env.exec("mv -n /src.txt /dst.txt"));
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe("");
      // Source should still exist since move was skipped
      const srcExists = await toText(await env.exec("cat /src.txt"));
    ...
    it("should show verbose output with -v flag", async () => {
      ...
      const result = await toText(await env.exec("mv -v /old.txt /new.txt"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle combined flags -fv", async () => {
      ...
      const result = await toText(await env.exec("mv -fv /src.txt /dst.txt"));
      expect(result.exitCode).toBe(0);
    ...
    it("should let -n take precedence over -f", async () => {
      ...
      const result = await toText(await env.exec("mv -fn /src.txt /dst.txt"));
      expect(result.exitCode).toBe(0);
    ...
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("mv --help"));
      expect(result.exitCode).toBe(0);
    ...
    it("should error on unknown flag", async () => {
      ...
      const result = await toText(await env.exec("mv -x /src.txt /dst.txt"));
      expect(result.exitCode).toBe(1);
