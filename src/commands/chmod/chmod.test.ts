...
describe("chmod command", () => {
  describe("octal mode", () => {
    it("should change file permissions with octal mode", async () => {
      ...
      const result = await toText(await env.exec("chmod 755 /test.txt"));
      expect(result.exitCode).toBe(0);

      // Verify mode changed via stat
      const statResult = await toText(await env.exec("stat -c %a /test.txt"));
  ...
  describe("symbolic mode", () => {
    it("should add execute permission with u+x", async () => {
      ...
      const statResult = await toText(await env.exec("stat -c %a /script.sh"));
  ...
  describe("multiple files", () => {
    it("should change permissions on multiple files", async () => {
      ...
      const result = await toText(await env.exec("chmod 600 /a.txt /b.txt"));
      expect(result.exitCode).toBe(0);

      const statA = await toText(await env.exec("stat -c %a /a.txt"));
      const statB = await toText(await env.exec("stat -c %a /b.txt"));
  ...
  describe("recursive mode", () => {
    it("should change permissions recursively with -R", async () => {
      ...
      const result = await toText(await env.exec("chmod -R 755 /dir"));
      expect(result.exitCode).toBe(0);

      const statA = await toText(await env.exec("stat -c %a /dir/a.txt"));
      const statB = await toText(await env.exec("stat -c %a /dir/sub/b.txt"));
  ...
  describe("error handling", () => {
    it("should error on missing operand", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("chmod"));
    ...
    it("should error on missing file", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("chmod 755 /nonexistent"));
      expect(result.exitCode).toBe(1);
    ...
    it("should error on invalid mode", async () => {
      ...
      const result = await toText(await env.exec("chmod xyz /test.txt"));
      expect(result.exitCode).toBe(1);
    ...
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("chmod --help"));
