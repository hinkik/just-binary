...
describe("fold", () => {
  it("wraps at 80 columns by default", async () => {
    ...
    const result = await toText(await bash.exec(`echo '${longLine}' | fold`));
  ...
  it("wraps at specified width with -w", async () => {
    ...
    const result = await toText(
      await bash.exec("echo 'hello world' | fold -w 5"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("wraps at specified width with -w attached", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("echo '1234567890' | fold -w5"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("breaks at spaces with -s when word fits exactly", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("echo 'abc defgh' | fold -sw 6"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("handles lines shorter than width", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("echo 'short' | fold -w 80"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles empty input", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("printf '' | fold"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles no trailing newline", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("printf 'hello' | fold -w 5"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("hello");
  });
  ...
  it("reads from file", async () => {
    ...
    const result = await toText(await bash.exec("fold -w 5 /test.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("12345\n67890\n");
  });
  ...
  it("reads from multiple files", async () => {
    ...
    const result = await toText(await bash.exec("fold -w 5 /a.txt /b.txt"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles file not found", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("fold /nonexistent.txt"));
  ...
  it("shows help with --help", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("fold --help"));
    expect(result.exitCode).toBe(0);
  ...
  it("errors on invalid width", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("echo 'x' | fold -w abc"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on zero width", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("echo 'x' | fold -w 0"));
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("invalid number of columns");
  });
  ...
  it("handles tabs expanding to 8-column boundary", async () => {
    ...
    const result = await toText(
      await bash.exec("printf '\\tabc' | fold -w 10"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("handles -s with no spaces in line", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("echo 'abcdefghij' | fold -sw 5"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("handles -s with space at exact boundary", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("echo 'abcd efgh' | fold -sw 5"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("errors on unknown short flag", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("echo 'x' | fold -x"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown long flag", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("echo 'x' | fold --unknown"));
    expect(result.exitCode).toBe(1);
