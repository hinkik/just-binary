...
describe("sleep command", () => {
  describe("basic functionality", () => {
    it("should sleep for specified seconds", async () => {
      ...
      const result = await toText(await env.exec("sleep 2"));
    ...
    it("should handle decimal seconds", async () => {
      ...
      const result = await toText(await env.exec("sleep 0.5"));
      expect(result.exitCode).toBe(0);
  ...
  describe("duration suffixes", () => {
    it("should handle seconds suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 3s"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle minutes suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 2m"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle hours suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 1h"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle days suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 1d"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle decimal values with suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 0.5m"));
      expect(result.exitCode).toBe(0);
  ...
  describe("multiple arguments", () => {
    it("should sum multiple durations", async () => {
      ...
      const result = await toText(await env.exec("sleep 1 2 3"));
      expect(result.exitCode).toBe(0);
    ...
    it("should sum durations with mixed suffixes", async () => {
      ...
      const result = await toText(await env.exec("sleep 1s 1m"));
      expect(result.exitCode).toBe(0);
  ...
  describe("error handling", () => {
    it("should error on missing operand", async () => {
      ...
      const result = await toText(await env.exec("sleep"));
    ...
    it("should error on invalid time interval", async () => {
      ...
      const result = await toText(await env.exec("sleep abc"));
      expect(result.exitCode).toBe(1);
    ...
    it("should error on invalid suffix", async () => {
      ...
      const result = await toText(await env.exec("sleep 1x"));
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain("invalid time interval");
    });
  });
  ...
  describe("help", () => {
    it("should show help with --help", async () => {
      ...
      const result = await toText(await env.exec("sleep --help"));
      expect(result.exitCode).toBe(0);
  ...
  describe("short duration sleep", () => {
    it("should handle very short sleep durations", async () => {
      ...
      const result = await toText(await env.exec("sleep 0.01")); // 10ms
      expect(result.exitCode).toBe(0);
