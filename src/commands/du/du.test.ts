...
describe("du command", () => {
  describe("basic usage", () => {
    it("should show directory size", async () => {
      ...
      const result = await toText(await env.exec("du /mydir"));
    ...
    it("should show file size", async () => {
      ...
      const result = await toText(await env.exec("du /test.txt"));
    ...
    it("should error on missing file", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("du /nonexistent"));
  ...
  describe("-a option", () => {
    it("should show all files with -a", async () => {
      ...
      const result = await toText(await env.exec("du -a /dir"));
  ...
  describe("-s option", () => {
    it("should show only summary with -s", async () => {
      ...
      const result = await toText(await env.exec("du -s /dir"));
      expect(result.stdout).toContain("/dir");
  ...
  describe("-h option", () => {
    it("should show human readable sizes", async () => {
      ...
      const result = await toText(await env.exec("du -h /big"));
  ...
  describe("-c option", () => {
    it("should show grand total", async () => {
      ...
      const result = await toText(await env.exec("du -c /dir1 /dir2"));
  ...
  describe("--max-depth option", () => {
    it("should limit depth with --max-depth=0", async () => {
      ...
      const result = await toText(await env.exec("du --max-depth=0 /dir"));
      const lines = result.stdout.trim().split("\n");
      expect(lines.length).toBe(1);
      expect(result.stdout).toContain("/dir");
    });
  ...
  describe("help option", () => {
    it("should show help with --help", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("du --help"));
