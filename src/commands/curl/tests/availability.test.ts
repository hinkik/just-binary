...
describe("curl availability", () => {
  describe("without network configuration", () => {
    it("curl command does not exist", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("curl https://example.com"));
    ...
    it("curl is not in /bin", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("ls /bin/curl"));
    ...
    it("curl does not appear in ls /bin output", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("ls /bin | grep ^curl$"));
  ...
  describe("with network configuration", () => {
    it("curl command exists", async () => {
      ...
      const result = await toText(await env.exec("curl --help"));
