...
describe("od command", () => {
  it("should dump stdin in octal format", async () => {
    const env = new Bash();
    const result = await toText(await env.exec('echo -n "AB" | od'));
  ...
  it("should show character mode with -c", async () => {
    const env = new Bash();
    const result = await toText(await env.exec('echo -n "hi" | od -c'));
  ...
  it("should show escape sequences in character mode", async () => {
    const env = new Bash();
    const result = await toText(await env.exec('echo "hello" | od -c'));
  ...
  it("should suppress addresses with -An", async () => {
    const env = new Bash();
    const result = await toText(await env.exec('echo -n "A" | od -An'));
  ...
  it("should read from file", async () => {
    ...
    const result = await toText(await env.exec("od /tmp/od-test.txt"));
  ...
  it("should error on non-existent file", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("od /nonexistent/file.txt"));
