...
describe("ls -h (human-readable)", () => {
  it("displays bytes for small files", async () => {
    ...
    const result = await toText(await env.exec("ls -lh /test"));
  ...
  it("works with --human-readable long form", async () => {
    ...
    const result = await toText(await env.exec("ls -l --human-readable /test"));
    expect(result.exitCode).toBe(0);
  ...
  it("displays exact bytes without -h flag", async () => {
    ...
    const result = await toText(await env.exec("ls -l /test"));
    expect(result.exitCode).toBe(0);
  ...
  it("can combine -h with other flags", async () => {
    ...
    const result = await toText(await env.exec("ls -lah /test"));
    expect(result.exitCode).toBe(0);
