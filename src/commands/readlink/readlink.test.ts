...
describe("readlink", () => {
  describe("basic usage", () => {
    it("should read symlink target", async () => {
      ...
      const result = await toText(await env.exec("readlink /tmp/link"));
    ...
    it("should handle multiple files", async () => {
      ...
      const result = await toText(
        await env.exec("readlink /tmp/link1 /tmp/link2"),
      );
    ...
    it("should fail for non-symlink file", async () => {
      ...
      const result = await toText(await env.exec("readlink /tmp/regular.txt"));
      expect(result.exitCode).toBe(1);
    });

    it("should fail for non-existent file", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("readlink /tmp/nonexistent"));
      expect(result.exitCode).toBe(1);
    });
  });
  ...
  describe("-f (canonicalize)", () => {
    it("should canonicalize path through symlinks", async () => {
      ...
      const result = await toText(await env.exec("readlink -f /tmp/link2"));
    ...
    it("should return path for regular file", async () => {
      ...
      const result = await toText(await env.exec("readlink -f /tmp/file.txt"));
    ...
    it("should canonicalize with relative symlink components", async () => {
      ...
      const result = await toText(await env.exec("readlink -f /tmp/link"));
    ...
    it("should return resolved path for nonexistent file with -f", async () => {
      const env = new Bash();
      const result = await toText(
        await env.exec("readlink -f /tmp/nonexistent"),
      );
  ...
  describe("error handling", () => {
    it("should error on missing operand", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("readlink"));
    ...
    it("should error on unknown option", async () => {
      const env = new Bash();
      const result = await toText(await env.exec("readlink -x /tmp/link"));
    ...
    it("should handle -- to end options", async () => {
      ...
      const result = await toText(await env.exec("readlink -- /tmp/-f"));
