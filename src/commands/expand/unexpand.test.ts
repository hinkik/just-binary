...
describe("unexpand", () => {
  it("converts leading spaces to tabs (default 8)", async () => {
    ...
    const result = await toText(
      await bash.exec("printf '        hello' | unexpand"),
    );
  ...
  it("does not convert spaces after text by default", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf 'hello        world' | unexpand"),
    );
    expect(result.exitCode).toBe(0);
  ...
  it("converts all spaces with -a", async () => {
    ...
    const result = await toText(
      await bash.exec("printf 'hello   world' | unexpand -a"),
    );
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("hello\tworld");
  });
  ...
  it("uses custom tab width with -t", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf '    hello' | unexpand -t 4"),
    );
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\thello");
  });
  ...
  it("handles empty input", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("printf '' | unexpand"));
    expect(result.exitCode).toBe(0);
  ...
  it("handles input with no spaces", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("printf 'hello' | unexpand"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("hello");
  });
  ...
  it("reads from file", async () => {
    ...
    const result = await toText(await bash.exec("unexpand /test.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\thello\n\tworld\n");
  });
  ...
  it("reads from multiple files", async () => {
    ...
    const result = await toText(await bash.exec("unexpand /a.txt /b.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\ta\n\tb\n");
  });
  ...
  it("handles file not found", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("unexpand /nonexistent.txt"));
  ...
  it("shows help with --help", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("unexpand --help"));
    expect(result.exitCode).toBe(0);
  ...
  it("errors on invalid tab size", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf 'x' | unexpand -t abc"),
    );
    expect(result.exitCode).toBe(1);
  ...
  it("handles --all flag", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf 'hello   world' | unexpand --all"),
    );
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("hello\tworld");
  });
  ...
  it("handles --tabs= option", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf '    hello' | unexpand --tabs=4"),
    );
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\thello");
  });
  ...
  it("errors on unknown short flag", async () => {
    const bash = new Bash();
    const result = await toText(await bash.exec("printf 'x' | unexpand -x"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown long flag", async () => {
    const bash = new Bash();
    const result = await toText(
      await bash.exec("printf 'x' | unexpand --unknown"),
    );
    expect(result.exitCode).toBe(1);
