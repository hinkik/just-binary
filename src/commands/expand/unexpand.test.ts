...
describe("unexpand", () => {
  it("converts leading spaces to tabs (default 8)", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf '        hello' | unexpand"));
  ...
  it("handles empty input", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf '' | unexpand"));
    expect(result.exitCode).toBe(0);
  ...
  it("reads from file", async () => {
    ...
    const result = toText(await bash.exec("unexpand /test.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\thello\n\tworld\n");
  });
  ...
  it("reads from multiple files", async () => {
    ...
    const result = toText(await bash.exec("unexpand /a.txt /b.txt"));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("\ta\n\tb\n");
  });
  ...
  it("handles file not found", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("unexpand /nonexistent.txt"));
  ...
  it("shows help with --help", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("unexpand --help"));
    expect(result.exitCode).toBe(0);
  ...
  it("errors on invalid tab size", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | unexpand -t abc"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown short flag", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | unexpand -x"));
    expect(result.exitCode).toBe(1);
  ...
  it("errors on unknown long flag", async () => {
    const bash = new Bash();
    const result = toText(await bash.exec("printf 'x' | unexpand --unknown"));
    expect(result.exitCode).toBe(1);
