...
describe("sed errors", () => {
  ...
  describe("file errors", () => {
    it("should error on non-existent file", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 's/a/b/' /nonexistent.txt"));
    ...
    it("should error on multiple non-existent files", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 's/a/b/' /no1.txt /no2.txt"));
      expect(result.stderr).toContain("No such file or directory");
      expect(result.exitCode).toBe(1);
    });
    ...
    it("should error on non-existent script file with -f", async () => {
      const env = createEnv();
      const result = toText(
        await env.exec("sed -f /nonexistent.sed /test/file.txt"),
      );
      expect(result.stderr).toContain("No such file or directory");
      expect(result.exitCode).toBe(1);
    });
  ...
  describe("script parsing errors", () => {
    it("should error on missing script", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed"));
    ...
    it("should handle unterminated substitution", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 's/foo/bar' /test/file.txt"));
    ...
    it("should handle unknown command gracefully", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 'z' /test/file.txt"));
    ...
    it("should handle unknown flag gracefully", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 's/a/b/z' /test/file.txt"));
  ...
  describe("address errors", () => {
    it("should handle line 0 address gracefully", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed '0d' /test/file.txt"));
    ...
    it("should error on malformed regex address", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed '/foo d' /test/file.txt"));
    ...
    it("should handle unknown POSIX class as literal", async () => {
      ...
      const result = toText(
        await env.exec("sed '/[[:invalid:]]/d' /test/file.txt"),
      );
  ...
  describe("label errors", () => {
    it("should error on branch to undefined label", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 'b undefined' /test/file.txt"));
    ...
    it("should error on t command with undefined label", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 't missing' /test/file.txt"));
      expect(result.stderr).toContain("undefined label");
      expect(result.exitCode).toBe(1);
    });
  });
  ...
  describe("option errors", () => {
    it("should error on unknown short option", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed -z 's/a/b/' /test/file.txt"));
    ...
    it("should error on unknown long option", async () => {
      const env = createEnv();
      const result = toText(
        await env.exec("sed --unknown 's/a/b/' /test/file.txt"),
      );
    ...
    it("should error on -e without argument", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed -e /test/file.txt"));
  ...
  describe("regex errors", () => {
    it("should error on invalid regex pattern", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 's/[/x/' /test/file.txt"));
    ...
    it("should error on invalid backreference", async () => {
      ...
      const result = toText(await env.exec("sed 's/foo/\\9/' /test/file.txt"));
    ...
    it("should handle unmatched parenthesis in BRE", async () => {
      ...
      const result = toText(
        await env.exec("sed 's/(foo)/[\\1]/' /test/file.txt"),
      );
  ...
  describe("y command errors", () => {
    it("should error on mismatched y command lengths", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 'y/abc/xy/' /test/file.txt"));
    ...
    it("should error on unterminated y command", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed 'y/abc/xyz' /test/file.txt"));
  ...
  describe("{ } block errors", () => {
    it("should handle valid block syntax", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed '1{d;}' /test/file.txt"));
      expect(result.exitCode).toBe(0);
    ...
    it("should handle nested blocks", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed '1{s/1/X/;}' /test/file.txt"));
      expect(result.exitCode).toBe(0);
  ...
  describe("step address errors", () => {
    it("should handle step address with step 0", async () => {
      const env = createEnv();
      const result = toText(await env.exec("sed '1~0d' /test/file.txt"));
