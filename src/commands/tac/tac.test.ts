...
describe("tac command", () => {
  ...
  it("should handle single line", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo "hello" | tac'));
  ...
  it("should handle empty input", async () => {
    const env = new Bash();
    const result = toText(await env.exec('echo -n "" | tac'));
  ...
  it("should read from file", async () => {
    ...
    const result = toText(await env.exec("tac /tmp/test.txt"));
  ...
  it("should error on non-existent file", async () => {
    const env = new Bash();
    const result = toText(await env.exec("tac /nonexistent/file.txt"));
