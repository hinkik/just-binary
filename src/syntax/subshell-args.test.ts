...
describe("Positional arguments and operator precedence", () => {
  describe("bash/sh positional arguments", () => {
    it("should handle bash -c with positional args (single quoted)", async () => {
      ...
      const result = toText(
        await env.exec("bash -c 'echo $1 $2' script arg1 arg2"),
      );
    ...
    it("should handle sh -c with positional args (single quoted)", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("sh -c 'echo $1 $2' script arg1 arg2"),
      );
      expect(result.stdout).toBe("arg1 arg2\n");
    });
    ...
    it("should set $0 to script name", async () => {
      const env = new Bash();
      const result = toText(await env.exec("bash -c 'echo $0' myscript"));
    ...
    it("should handle script file with positional args", async () => {
      ...
      const result = toText(await env.exec("bash /script.sh one two three"));
      expect(result.stdout).toBe("Args: one two three\n");
    });

    it("should set $# to argument count", async () => {
      const env = new Bash();
      const result = toText(await env.exec("bash -c 'echo $#' script a b c"));
  ...
  describe("xargs positional arguments", () => {
    it("should append args to command", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo "a b c" | xargs echo prefix'));
    ...
    it("should handle null-separated input with -0", async () => {
      ...
      const result = toText(
        await env.exec('printf "file1\\x00file2\\x00file3" | xargs -0 echo'),
      );
  ...
  describe("Operator precedence", () => {
    it("! should bind tighter than &&", async () => {
      ...
      const result = toText(await env.exec("! false && echo yes"));
    ...
    it("! should negate entire pipeline", async () => {
      ...
      const result = toText(await env.exec("! echo hello | grep missing"));
    ...
    it("! should negate successful pipeline", async () => {
      ...
      const result = toText(await env.exec("! echo hello | grep hello"));
    ...
    it("&& and || should be left-associative", async () => {
      ...
      const result = toText(await env.exec("true || echo no && echo yes"));
      expect(result.stdout).toBe("yes\n");
    });
    ...
    it("; should have lowest precedence", async () => {
      ...
      const result = toText(await env.exec("false && echo no ; echo always"));
