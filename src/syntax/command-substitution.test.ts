...
describe("Command Substitution $(cmd)", () => {
  it("should capture echo output", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $(echo hello)"));
  ...
  it("should capture command output in variable (within same exec)", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("X=$(echo world); echo $X"));
  ...
  it("should work with cat", async () => {
    ...
    const result = await toText(await env.exec("echo $(cat /test.txt)"));
  ...
  it("should strip trailing newline from substitution", async () => {
    ...
    const result = await toText(
      await env.exec("echo prefix-$(echo middle)-suffix"),
    );
  ...
  it("should handle nested command substitution", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $(echo $(echo nested))"));
  ...
  it("should work with pipes inside substitution", async () => {
    ...
    const result = await toText(
      await env.exec("echo $(cat /test.txt | grep world)"),
    );
    expect(result.stdout).toBe("world\n");
  });
  ...
  it("should work with wc -l", async () => {
    ...
    const result = await toText(
      await env.exec("echo lines: $(wc -l < /test.txt)"),
    );
  ...
  it("should work in variable assignment (within same exec)", async () => {
    const env = new Bash();
    const result = await toText(
      await env.exec("COUNT=$(echo 42); echo $COUNT"),
    );
  ...
  it("should handle empty command output", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo prefix$(echo)suffix"));
...
describe("Arithmetic Expansion $((expr))", () => {
  it("should evaluate simple addition", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $((1 + 2))"));
    expect(result.stdout).toBe("3\n");
  });

  it("should evaluate subtraction", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $((10 - 3))"));
    expect(result.stdout).toBe("7\n");
  });

  it("should evaluate multiplication", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $((4 * 5))"));
  ...
  it("should evaluate power", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $((2 ** 8))"));
  ...
  it("should handle variables with $ prefix", async () => {
    ...
    const result = await toText(await env.exec("echo $(($X + 3))"));
  ...
  it("should handle comparison operators", async () => {
    const env = new Bash();
    expect((await toText(await env.exec("echo $((5 > 3))"))).stdout).toBe(
      "1\n",
    );
    expect((await toText(await env.exec("echo $((5 < 3))"))).stdout).toBe(
      "0\n",
    );
    expect((await toText(await env.exec("echo $((5 == 5))"))).stdout).toBe(
      "1\n",
    );
    expect((await toText(await env.exec("echo $((5 != 5))"))).stdout).toBe(
      "0\n",
  ...
  it("should handle logical operators", async () => {
    const env = new Bash();
    expect((await toText(await env.exec("echo $((1 && 1))"))).stdout).toBe(
      "1\n",
    );
    expect((await toText(await env.exec("echo $((1 && 0))"))).stdout).toBe(
      "0\n",
    );
    expect((await toText(await env.exec("echo $((0 || 1))"))).stdout).toBe(
      "1\n",
    );
    expect((await toText(await env.exec("echo $((0 || 0))"))).stdout).toBe(
      "0\n",
    );
  });
  ...
  it("should handle shift operators", async () => {
    const env = new Bash();
    expect((await toText(await env.exec("echo $((1 << 4))"))).stdout).toBe(
      "16\n",
    );
    expect((await toText(await env.exec("echo $((16 >> 2))"))).stdout).toBe(
      "4\n",
    );
  });
  ...
  it("should handle complex expressions", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("echo $((2 + 3 * 4 - 1))"));
    expect(result.stdout).toBe("13\n"); // 2 + 12 - 1 = 13
  });

  it("should work in variable assignment (within same exec)", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("SUM=$((10 + 20)); echo $SUM"));
    expect(result.stdout).toBe("30\n");
  });

  it("should handle increment pattern (within same exec)", async () => {
    const env = new Bash();
    const result = await toText(await env.exec("N=5; N=$((N + 1)); echo $N"));
