...
describe("Bash Syntax - Parser Edge Cases", () => {
  describe("quoting", () => {
    ...
    it("should handle quotes inside arguments", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo foo'bar'baz"));
  ...
  describe("variable expansion", () => {
    it("should handle ${VAR:-default} with set variable", async () => {
      ...
      const result = toText(await env.exec('echo "${VAR:-default}"'));
    ...
    it("should handle $VAR with no braces", async () => {
      ...
      const result = toText(await env.exec("echo $NAME"));
    ...
    it("should handle variable followed by text", async () => {
      const env = new Bash({ env: { NAME: "test" } });
      const result = toText(await env.exec('echo "${NAME}file.txt"'));
      expect(result.stdout).toBe("testfile.txt\n");
    });

    it("should handle undefined variable as empty", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo "[$UNDEFINED]"'));
      expect(result.stdout).toBe("[]\n");
    });
  ...
  describe("whitespace handling", () => {
    it("should handle multiple spaces between arguments", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo    a    b    c"));
  ...
  describe("redirection parsing", () => {
    ...
    it("should handle 2>/dev/null", async () => {
      const env = new Bash();
      const result = toText(await env.exec("cat /nonexistent 2>/dev/null"));
    ...
    it("should handle 2>&1 redirection", async () => {
      const env = new Bash();
      const result = toText(await env.exec("cat /nonexistent 2>&1"));
  ...
  describe("operator parsing", () => {
    it("should parse && correctly without spaces", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo a&&echo b"));
      expect(result.stdout).toBe("a\nb\n");
    });

    it("should parse || correctly without spaces", async () => {
      const env = new Bash();
      const result = toText(await env.exec("false||echo fallback"));
    ...
    it("should differentiate & from &&", async () => {
      ...
      const env = new Bash();
      const result = toText(await env.exec("true && echo success"));
  ...
  describe("complex command combinations", () => {
    it("should handle mixed && and || with correct precedence", async () => {
      ...
      const result = toText(await env.exec("false || echo A && echo B"));
      expect(result.stdout).toBe("A\nB\n");
    });

    it("should handle semicolon with && and ||", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo a; false || echo b; echo c"));
      expect(result.stdout).toBe("a\nb\nc\n");
    });
    ...
    it("should handle assignment followed by command", async () => {
      const env = new Bash();
      const result = toText(await env.exec("x=hello; echo $x"));
      expect(result.stdout).toBe("hello\n");
    });
  ...
  describe("edge cases", () => {
    ...
    it("should handle very long argument", async () => {
      ...
      const result = toText(await env.exec(`echo ${longStr}`));
      expect(result.stdout).toBe(`${longStr}\n`);
    });

    it("should handle unicode in arguments", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo "Hello 世界 🌍"'));
