...
describe("Bash Syntax - Parse Errors", () => {
  describe("if statement errors", () => {
    ...
    it("should error on missing then", async () => {
      const env = new Bash();
      const result = toText(await env.exec("if true; echo hello; fi"));
      expect(result.exitCode).not.toBe(0);
    });
    ...
    it("should handle elif with semicolon as condition", async () => {
      ...
      const result = toText(
        await env.exec("if false; then echo a; elif true; then echo b; fi"),
      );
    ...
    it("should error on else without if", async () => {
      const env = new Bash();
      const result = toText(await env.exec("else echo hello; fi"));
      expect(result.exitCode).toBe(2); // syntax error
    });

    it("should error on fi without if", async () => {
      const env = new Bash();
      const result = toText(await env.exec("fi"));
      expect(result.exitCode).toBe(2); // syntax error
    });
  });
  ...
  describe("for loop errors", () => {
    it("should error on missing in keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("for x a b c; do echo $x; done"));
    ...
    it("should error on missing do keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("for x in a b c; echo $x; done"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
    ...
    it("should error on missing done keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("for x in a b c; do echo $x"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
    ...
    it("should error on invalid variable name", async () => {
      const env = new Bash();
      const result = toText(
        await env.exec("for 123 in a b c; do echo $123; done"),
      );
  ...
  describe("while loop errors", () => {
    it("should error on missing do keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("while true; echo loop; done"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
    ...
    it("should error on missing done keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("while true; do echo loop"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
    ...
    it("should error on while followed by semicolon", async () => {
      ...
      const result = toText(await env.exec("while; do echo loop; done"));
      expect(result.exitCode).toBe(2); // Syntax error
      expect(result.stderr).toContain("syntax error");
    });
  });
  ...
  describe("until loop errors", () => {
    it("should error on missing do keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("until true; echo loop; done"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
    ...
    it("should error on missing done keyword", async () => {
      const env = new Bash();
      const result = toText(await env.exec("until true; do echo loop"));
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain("syntax error");
    });
  });
  ...
  describe("function definition errors", () => {
    it("should accept function with numeric-starting name", async () => {
      ...
      const result = toText(await env.exec("123func() { echo hello; }"));
      expect(result.exitCode).toBe(0);
    });

    it("should error on unclosed function body", async () => {
      const env = new Bash();
      const result = toText(await env.exec("myfunc() { echo hello"));
  ...
  describe("quote errors", () => {
    it("should handle unclosed double quote gracefully", async () => {
      ...
      const result = toText(await env.exec('echo "unclosed'));
  ...
  describe("redirection errors", () => {
    it("should auto-create parent directories on redirect", async () => {
      ...
      const result = toText(await env.exec("echo test > /newdir/file.txt"));
      expect(result.exitCode).toBe(0);
    ...
    it("should error on redirect without target", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo test >"));
  ...
  describe("command errors", () => {
    ...
    it("should return 127 for command path not found", async () => {
      const env = new Bash();
      const result = toText(await env.exec("/nonexistent/path/command"));
      expect(result.exitCode).toBe(127);
  ...
  describe("local keyword errors", () => {
    it("should error when local used outside function", async () => {
      const env = new Bash();
      const result = toText(await env.exec("local x=1"));
      expect(result.exitCode).toBe(1);
  ...
  describe("pipe and operator errors", () => {
    it("should handle empty command before pipe", async () => {
      const env = new Bash();
      const result = toText(await env.exec("| cat"));
