...
describe("Bash Syntax - Variables and Quoting", () => {
  describe("environment variable expansion", () => {
    ...
    it("should handle unset variable as empty", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo "[$UNSET]"'));
    ...
    it("should expand in double quotes", async () => {
      ...
      const result = toText(await env.exec('echo "the $VAR is here"'));
    ...
    it("should handle unset command (within same exec)", async () => {
      ...
      const result = toText(await env.exec('unset FOO; echo "[$FOO]"'));
      expect(result.stdout).toBe("[]\n");
    });
  });
  ...
  describe("quoting", () => {
    ...
    it("should preserve special chars in single quotes", async () => {
      const env = new Bash();
      const result = toText(await env.exec("echo 'hello $VAR && test'"));
      expect(result.stdout).toBe("hello $VAR && test\n");
    });

    it("should handle newline in quoted string with $", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo "line1\nline2"'));
  ...
  describe("escape sequences", () => {
    it("should handle \\n with echo -e", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo -e "hello\\nworld"'));
      expect(result.stdout).toBe("hello\nworld\n");
    });

    it("should handle \\t with echo -e", async () => {
      const env = new Bash();
      const result = toText(await env.exec('echo -e "col1\\tcol2"'));
  ...
  describe("unknown commands", () => {
    ...
    it("should include command name in error", async () => {
      const env = new Bash();
      const result = toText(await env.exec("foobar"));
  ...
  describe("variable assignments", () => {
    it("should assign value with double quotes", async () => {
      const env = new Bash();
      const result = toText(await env.exec('MYVAR="hello"; echo $MYVAR'));
      expect(result.stdout).toBe("hello\n");
    });
    ...
    it("should assign empty string with double quotes", async () => {
      const env = new Bash();
      const result = toText(await env.exec('MYVAR=""; echo "value:$MYVAR:"'));
      expect(result.stdout).toBe("value::\n");
