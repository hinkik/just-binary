...
`,
  ...
  it('should find all CommonJS require statements', async () => {
    const env = createEnv();
    const result = await env.exec('grep -rn "require(" /project/src --include="*.js"');
    expect(result.stdout).toBe(`/project/src/config.js:1:const path = require('path');
...
`);
  ...
  it('should find all module.exports statements', async () => {
    const env = createEnv();
    const result = await env.exec('grep -rn "module.exports" /project/src --include="*.js"');
    expect(result.stdout).toBe(`/project/src/config.js:11:module.exports = config;
...
`);
  ...
  it('should identify external vs internal dependencies', async () => {
    ...
    const external = await env.exec('grep -rh "require(" /project/src --include="*.js" | grep -v "require(\'\\.\\.\\|require(\'\\./" | sort | uniq');
...
`);
  ...
  it('should convert require to import using sed', async () => {
    ...
    await env.exec("sed \"s/const express = require\\('express'\\);/import express from 'express';/g\" /project/src/index.js > /project/src/index.js.new");
  ...
  it('should rename .js to .mjs files', async () => {
    ...
    await env.exec('mv /project/src/utils/helpers.js /project/src/utils/helpers.mjs');
  ...
  it('should update package.json type to module', async () => {
    ...
    const before = await env.exec('grep "type" /project/package.json || echo "No type field"');
    expect(before.stdout).toBe('No type field\n');

    // Add type: module - directly write the modified content
    await env.exec('echo \'{"name":"legacy-app","type":"module"}\' > /project/package.json');
  ...
  it('should update tsconfig module to ESNext', async () => {
    ...
    await env.exec('sed \'s/"module": "CommonJS"/"module": "ESNext"/\' /project/tsconfig.json > /project/tsconfig.json.new');
  ...
  it('should find files with mixed export patterns', async () => {
    ...
    const result = await env.exec('grep -rl "module.exports =" /project/src --include="*.js" | sort | uniq');
...
`);
  ...
  it('should identify circular dependency risks', async () => {
    ...
    const dbImports = await env.exec("grep -l \"require.*logger\" /project/src/db.js");
    expect(dbImports.stdout).toBe('/project/src/db.js\n');

    // Check if server.js also imports logger
    const serverImports = await env.exec("grep -l \"require.*logger\" /project/src/server.js");
  ...
  it('should generate migration summary report', async () => {
    ...
    const requireCount = await env.exec('grep -r "require(" /project/src --include="*.js" | wc -l');
    const exportsCount = await env.exec('grep -r "module.exports" /project/src --include="*.js" | wc -l');
  ...
  it('should find relative imports that need extension updates', async () => {
    const env = createEnv();
    const result = await env.exec("grep -rh \"require('\\./\\|require('../\" /project/src | sort | uniq");
