...
`,
  ...
  it("should list config files", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("ls /app/config"));
  ...
  it("should read default configuration", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("cat /app/config/default.json"));
  ...
  it("should read production configuration", async () => {
    ...
    const result = await toText(
      await env.exec("cat /app/config/production.json"),
    );
    expect(result.stdout).toBe(`{
  "port": 8080,
  "database": {
...
`);
  ...
  it("should find all database hosts across configs", async () => {
    const env = createEnv();
    const result = await toText(await env.exec('grep -r "host" /app/config'));
    expect(
      result.stdout,
...
`);
  ...
  it("should find cache settings", async () => {
    const env = createEnv();
    const result = await toText(
      await env.exec('grep -r "enabled" /app/config'),
    );
...
`);
  ...
  it("should list env files", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("ls -a /app"));
  ...
  it("should read env example file", async () => {
    const env = createEnv();
    const result = await toText(await env.exec("cat /app/.env.example"));
...
`);
  ...
  it("should count config lines", async () => {
    const env = createEnv();
    const result = await toText(
      await env.exec("wc -l /app/config/default.json"),
    );
  ...
  it("should find differences in port settings", async () => {
    const env = createEnv();
    const result = await toText(
      await env.exec("grep 3000 /app/config/default.json"),
    );
    expect(result.stdout).toBe('  "port": 3000,\n');
    expect(result.stderr).toBe("");
    expect(result.exitCode).toBe(0);
...
`);
  ...
  it("should filter env lines with awk pattern", async () => {
    const env = createEnv();
    const result = await toText(
      await env.exec("awk '/URL/' /app/.env.example"),
    );
    expect(result.stdout).toBe(
