...
`,
  ...
  it("should list config files", async () => {
    const env = createEnv();
    const result = toText(await env.exec("ls /app/config"));
  ...
  it("should read default configuration", async () => {
    const env = createEnv();
    const result = toText(await env.exec("cat /app/config/default.json"));
  ...
  it("should read production configuration", async () => {
    const env = createEnv();
    const result = toText(await env.exec("cat /app/config/production.json"));
    expect(result.stdout).toBe(`{
  "port": 8080,
  "database": {
    "host": "db.prod.example.com",
  ...
  it("should find port configurations", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec('grep "port" /app/config/default.json'),
    );
...
`);
  ...
  it("should find all database hosts across configs", async () => {
    const env = createEnv();
    const result = toText(await env.exec('grep -r "host" /app/config'));
    expect(
      result.stdout,
...
`);
  ...
  it("should find cache settings", async () => {
    const env = createEnv();
    const result = toText(await env.exec('grep -r "enabled" /app/config'));
...
`);
  ...
  it("should list env files", async () => {
    const env = createEnv();
    const result = toText(await env.exec("ls -a /app"));
  ...
  it("should read env example file", async () => {
    const env = createEnv();
    const result = toText(await env.exec("cat /app/.env.example"));
...
`);
  ...
  it("should count config lines", async () => {
    const env = createEnv();
    const result = toText(await env.exec("wc -l /app/config/default.json"));
  ...
  it("should find differences in port settings", async () => {
    const env = createEnv();
    const result = toText(await env.exec("grep 3000 /app/config/default.json"));
    expect(result.stdout).toBe('  "port": 3000,\n');
    expect(result.stderr).toBe("");
    expect(result.exitCode).toBe(0);
  });
  ...
  it("should verify production uses different port", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("grep 8080 /app/config/production.json"),
    );
  ...
  it("should extract env variable names using awk with = separator", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("awk -F= '{print $1}' /app/.env.example"),
    );
...
`);
  ...
  it("should extract env variable values using awk", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("awk -F= '{print $2}' /app/.env.example"),
    );
...
`);
  ...
  it("should extract production env values", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("awk -F= '{print $2}' /app/.env.production"),
    );
...
`);
  ...
  it("should format env as key: value using awk", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("awk -F= '{print $1 \": \" $2}' /app/.env.example"),
    );
    expect(result.stdout).toBe(`DATABASE_URL: postgresql://localhost:5432/app
API_KEY: your-api-key-here
DEBUG: false
`);
  ...
  it("should count env variables with awk NR and END", async () => {
    const env = createEnv();
    const result = toText(
      await env.exec("awk 'END{print NR}' /app/.env.example"),
    );
  ...
  it("should filter env lines with awk pattern", async () => {
    const env = createEnv();
    const result = toText(await env.exec("awk '/URL/' /app/.env.example"));
    expect(result.stdout).toBe(
